...
public class ExecuteSearchCorsoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Corso modelloPerRicerca = UtilityCorsoForm.createCorsoFromParams(nomeInputParam, docenteInputParam, numeroIscrittiStringParam, dataInizioInputStringParam, dataFineStringParam);


		try {
			request.setAttribute("listCorsiAttribute", MyServiceFactory.getCorsoServiceInstance().findByExample(modelloPerRicerca));
			} catch (Exception e) {
				e.printStackTrace();
